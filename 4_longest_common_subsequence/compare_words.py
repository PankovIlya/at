"""
  Spelling Corrector - Expansion Algorithm sample
  generates all possible words distance from the search string (delete, insert, substitution)
  options do not need to be stored in memory, check immediate
  O((m*A)^k*m*log(n))
    A - lehgth alphabet
    k - number of errors
    n* - length  dict
  ...
  It is not effective if the number of errors > 1
  see module ngramms
"""
import time
